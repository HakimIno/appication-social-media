...
const Switch = ({ setTheme, theme, setThemeSwitch, themeSwitch }: Props) => {
    ...
    const vibrateOnSwitchIn = () => {
        Vibration.vibrate(35);
    };

    const vibrateOnSwitchOut = () => {
        Vibration.vibrate(25);
    };

    const translateAnimation = useAnimatedStyle(() => {
        return {
            transform: [{ translateX: withTiming(translateX.value, { duration: 50 }) }],
        };
    });
    ...
    return (
        <Animated.View
            ...
            <Pressable
                style={styles.button}
                onPressIn={vibrateOnSwitchIn}
                onPressOut={vibrateOnSwitchOut}
                onPress={() => {
